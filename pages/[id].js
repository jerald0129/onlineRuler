...
export default function Home() {
	...
	const markerPosition = router.query.id;
	let BL_cm = 0.5;
	let BL_inch = 129.5;
	let begin_x = 20;
		...
		const handleMouseDown = (e) => {
			if (typeof begin_x != "undefined" && begin_x !== null) {
				...
				if (markerPosition) {
					...
					const match = markerPosition.match(pattern);
					if (match) {
						...
						switch (unit.toLowerCase()) {
							case "cm":
								drawMark(parseInt((numericValue * 100.7) / 2.54));
								break;
							case "inch":
								drawMark(parseInt(numericValue * 100.7));
								break;
							case "mm":
								drawMark(parseInt((numericValue * 100.7) / 2.54 / 10));
								break;
							default:
								setMarkerPosition(null); // Handle unsupported units
						}
					}
				}
	...
	}, [router.query.id, markerPosition]);
	...
	return (
		<div>
			...
			{markerPosition &&
				!markerPosition.match(/^(\d+(?:\.\d+)?)\s*(cm|inch|mm)$/i) && (
					<div>Incorrect path</div>
				)}
		</div>
	);
}
